...
{
    public class DeleteListGiayBaoHongByIdAction
    {
        public string GiayBaoHongIds { get; set; }
        #region private
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteListGiayBaoHongByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.GiayBaoHongId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("BaoHongId không hợp lệ");
                }
            }
        }
