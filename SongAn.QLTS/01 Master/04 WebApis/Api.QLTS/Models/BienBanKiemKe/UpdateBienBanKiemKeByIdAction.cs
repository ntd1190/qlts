...
{
    public class UpdateBienBanKiemKeByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                if (_kiemKeId < 1)
                {
                    return returnActionError(HttpStatusCode.BadRequest, "Không tìm thấy KiemKeId");
                }

                var bizHeader = new UpdateBienBanKiemKeByIdBiz(context);
                

                bizHeader.BienBanKiemKeId = _kiemKeId;
        ...
        private void init()
        {
            ...
            MyTable_KiemKiemChiTiet.Columns.Add("SoLuongKiemKe", typeof(decimal));
            foreach (var item in _listChiTiet)
            {
                MyTable_KiemKiemChiTiet.Rows.Add(item.BienBanKiemKeId, item.TaiSanId, item.SoLuong, item.SoLuongKiemKe);
            }
            ...
            _kiemKeId = Protector.Int(kiemKeId, 0);
