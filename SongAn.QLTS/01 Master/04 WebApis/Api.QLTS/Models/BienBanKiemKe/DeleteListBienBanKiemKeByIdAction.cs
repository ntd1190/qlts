...
{
    public class DeleteListBienBanKiemKeByIdAction
    {
        public string KiemKeIds { get; set; }
        #region private
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteListBienBanKiemKeByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.BienBanKiemKeId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("KiemKeId không hợp lệ");
                }
            }
        }
