...
{
    public class GetListGhiTangByProjectionAction
    {
        ...
        {
            GetListGhiTangByCriteriaBiz biz = new GetListGhiTangByCriteriaBiz(context);
            ...
            try
            {
                ...
                IEnumerable<dynamic> listGhiTang = await biz.Execute();
                if (listGhiTang.Count() > 0)
                {
                    var obj = listGhiTang.FirstOrDefault();
                ...
                return ActionHelper.returnActionResult(HttpStatusCode.OK, listGhiTang, _metaData);
            }
            catch (Exception ex)
            {
