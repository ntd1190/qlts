...
{
    public class DeleteListBaoDuongByIdAction
    {
        public string BaoDuongIds { get; set; }
        #region private
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteListBaoDuongByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.BaoDuongId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("BaoDuongId không hợp lệ");
                }
            }
        }
