...
{
    public class GetListGhiGiamByProjectionAction
    {
        ...
        {
            GetListGhiGiamByCriteriaBiz biz = new GetListGhiGiamByCriteriaBiz(context);
            ...
            try
            {
                 ...
                 * fixed input
                 * ========================= */
                sortName = string.IsNullOrEmpty(sortName) ? "GhiGiamId" : sortName;
                sortDir = string.IsNullOrEmpty(sortDir) ? "asc" : sortDir;
                _length = _length < 1 ? 10 : _length;
                var orderClause = sortName + " " + sortDir;
                var total = 0;
                biz.PhongBanId = PhongBanId;
                biz.SoChungTu = SoChungTu;
                ...
                IEnumerable<dynamic> listGhiGiam = await biz.Execute();
                if (listGhiGiam.Count() > 0)
                {
                    var obj = listGhiGiam.FirstOrDefault();
                ...
                return ActionHelper.returnActionResult(HttpStatusCode.OK,listGhiGiam, _metaData);
            }
            catch (Exception ex)
            {
