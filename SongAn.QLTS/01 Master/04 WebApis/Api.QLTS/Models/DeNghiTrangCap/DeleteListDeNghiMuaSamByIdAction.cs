...
{
    public class DeleteListDeNghiMuaSamByIdAction
    {
        public string DeNghiIds { get; set; }
        #region private
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteListDeNghiTrangCapByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.DeNghiId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("DeNghiId không hợp lệ");
                }
            }
        }
