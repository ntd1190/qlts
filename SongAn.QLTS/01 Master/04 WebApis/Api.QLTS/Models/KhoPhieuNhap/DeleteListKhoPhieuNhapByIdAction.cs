...
{
    public class DeleteListKhoPhieuNhapByIdAction
    {
        public string KhoPhieuNhapIds { get; set; }
        #region private
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteListKhoPhieuNhapByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.KhoPhieuNhapId = Protector.Int(_listId[i]);
        ...
        private void init()
        {
            var _ids = KhoPhieuNhapIds.Split(',');
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("KhoPhieuNhapId không hợp lệ");
                }
            }
        }
