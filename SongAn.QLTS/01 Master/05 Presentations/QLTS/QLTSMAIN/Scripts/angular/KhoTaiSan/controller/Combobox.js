...
app.controller('KhoTaiSanCombobox', function ($rootScope, $scope, KhoTaiSanService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'KhoTaiSanCombobox';
    vm.NameId = 'cbxKhoTaiSan';
    vm.data = {
        KhoTaiSanList: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxKhoTaiSan.cbxKhoTaiSan.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.KhoTaiSan);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        KhoTaiSanService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
