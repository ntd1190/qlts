(function () {
    ...
    module.controller('KhoPhieuXuatEditCtrl', function ($scope, KhoPhieuXuatService, $q, utility, $timeout) {
        ...
        vm.action.checkSoLuongChiTiet = function (item) {
            ...
            if (item.SoLuong > item.SoLuongTon && item.SoLuongTon > 0) {
        ...
        vm.action.In = function () {
            $('#reportmodal').find('iframe').attr('src', '../../../QLTSMAIN/CrystalReport/ReportPage.aspx?name=rptPhieuXuatChuyenById&data=' + khoPhieuXuatId);
        ...
        function checkListChiTiet() {
            ...
            var has_error = false;
            if (vm.data.listChiTiet.length < 1)
            {
