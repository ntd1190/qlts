(function () {
    ...
    angular.module('app')
        .controller('KhachHangEditCtrl', controller)

    function controller($rootScope, $scope, KhachHangService, $window, utility) {
        var KhachHangId = 0;
        ...
        function activate() {
            $('#KhachHangEditPopup').on('hidden.bs.collapse', function () {
                KhachHangId = 0;
                getById(KhachHangId);
                $rootScope.isOpenPopup = false;
            });

            $('#KhachHangEditPopup').on('shown.bs.collapse', function () {
                //set focus                
                $window.document.getElementById('txtMa').focus();
                $rootScope.isOpenPopup = true;
            });

            $scope.$on('KhachHangEditCtrl.KhachHangId', function (event, data) {
                KhachHangId = data;
                refresh();
                setEnableButton();
            });
        ...
        function edit() {
            vm.status.isInValidMa = utility.checkInValid(vm.data.objKhachHang.MaKhachHang, 'isCode');
            if (vm.status.isInValidMa) {
                $("#txtMa").focus();
                return;
            }
            vm.status.isInValidTen = utility.checkInValid(vm.data.objKhachHang.TenKhachHang, 'isEmpty');
            ...
            KhachHangService.update(vm.data.objKhachHang).then(function (success) {
                if (success.data.data) {
                    ...
                    $rootScope.$broadcast('sa.qltsmain.KhachHang.KhachHang.reload');
                }
             ...
             vm.status.isLoading = false;
                $('#KhachHangEditPopup').collapse('hide');
        ...
        function add() {
            ...
            vm.status.isLoading = true;
            vm.data.objKhachHang.CoSoId = vm.data.CoSoId;
            vm.data.objKhachHang.NguoiTao = vm.data.UserLoginId;
        ...
        function getById(id) {
            if (id > 0) {
                vm.status.isLoading = true;
                KhachHangService.getById(id).then(function (success) {
                    if (success.data) {
                        vm.data.objKhachHang = success.data.data;
                    }
                    vm.status.isLoading = false;
