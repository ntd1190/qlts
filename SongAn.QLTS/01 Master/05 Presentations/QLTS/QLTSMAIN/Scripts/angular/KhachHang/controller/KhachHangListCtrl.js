(function () {
    ...
    angular.module("app")
        .controller("KhachHangListCtrl", controller)

    function controller($rootScope, $scope, KhachHangService) {
        var vm = this;
        //HOT-KEY       
        vm.keys = {
            ...
            F2: function (name, code) {
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    ...
                    $('#KhachHangEditPopup').collapse('show');
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.KhachHang.KhachHang.reload', function (event) {
                getPage(_tableState);
            });
        }
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.KhachHangListDisplay.length; i++) {
                var KhachHang = vm.data.KhachHangListDisplay[i];
        ...
        function getPage(tableState) {
            ...
            KhachHangService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId).then(function (success) {
        ...
        function edit(id) {
            $rootScope.$broadcast('KhachHangEditCtrl.KhachHangId', id);
            $('#KhachHangEditPopup').collapse('show');
            $("#txtMa").focus();
            $rootScope.isOpenPopup = true;
        }
        ...
        function apDung() {
            ...
            var KhachHangListDisplay = vm.data.KhachHangListDisplay;
            for (var i = 0; i < KhachHangListDisplay.length; i++) {
                if (KhachHangListDisplay[i].isSelected) {
                    selectedList.push(KhachHangListDisplay[i]);
                }
            }
            console.log(selectedList);
            $scope.$emit('KhachHangListPopup.action.ap-dung', selectedList);
        }
