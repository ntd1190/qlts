(function () {
    ...
    angular.module("app")
        .controller("CoSoListCtrl", controller)

    function controller($rootScope, $scope, CoSoService) {
        var vm = this;
        //HOT-KEY       
        vm.keys = {
            ...
            F2: function (name, code) {
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    $rootScope.$broadcast('CoSoEditCtrl.CoSoId', 0);
                    $rootScope.$broadcast('CoSoEditCtrl.onInitView', 0);
                    $('#CoSoEditPopup').collapse('show');
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.CoSo.CoSo.reload', function (event) {
                getPage(_tableState);
            });
        }
        ...
        function deleteSelected() {
            ...
            var CoSoSelected = new Array();

            for (var i = 0; i < vm.data.CoSoListDisplay.length; i++) {
                var CoSo = vm.data.CoSoListDisplay[i];
                if (CoSo.isSelected) {
                    CoSoSelected.push(CoSo.CoSoId);
                }
            }
            var ids = CoSoSelected.join(',');

            CoSoService.removeList(ids).then(function (success) {
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'CoSoId';
            ...
            CoSoService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.CoSoListDisplay = success.data.data;
        ...
        function edit(id) {
            $rootScope.$broadcast('CoSoEditCtrl.CoSoId', id);
            $('#CoSoEditPopup').collapse('show');
            $("#txtMa").focus();
            $rootScope.isOpenPopup = true;
        }
        ...
        function apDung() {
            ...
            var CoSoListDisplay = vm.data.CoSoListDisplay;
            for (var i = 0; i < CoSoListDisplay.length; i++) {
                if (CoSoListDisplay[i].isSelected) {
                    selectedList.push(CoSoListDisplay[i]);
                }
            }
            console.log(selectedList);
            $scope.$emit('CoSoListPopup.action.ap-dung', selectedList);
        }
