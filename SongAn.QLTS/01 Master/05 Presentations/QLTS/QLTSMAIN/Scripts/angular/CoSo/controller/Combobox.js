...
app.controller('CoSoCombobox', function ($rootScope, $scope, CoSoService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'CoSoCombobox';
    vm.NameId = 'cbxCoSo';
    vm.data = {
        CoSoList: [],
        CoSoListDisplay: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxCoSo.cbxCoSo.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.CoSo);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        CoSoService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
