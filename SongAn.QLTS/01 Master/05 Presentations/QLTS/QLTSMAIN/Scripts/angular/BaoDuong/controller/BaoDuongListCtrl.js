(function () {
    ...
    angular.module("app")
        .controller("BaoDuongListCtrl", controller)
    ...
    function controller($rootScope, $scope, BaoDuongService, TuyChonCotService, KhoaSoLieuService, $q, utility) {
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                if (vm.data.listQuyenTacVu.indexOf("N") > 0) {
                    window.location.href = vm.data.linkUrl + 'BaoDuong/edit?id=0';
                }
            },
        ...
        function loadCotList() {
            if (1 === 1) {
                TuyChonCotService.getAll('FL0023',vm.data.userInfo.UserId).then(function (success) {
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.BaoDuong.BaoDuong.reload', function (event) {
                getPage(_tableState);
            });

            $scope.$on('BaoDuongListCtrl.action.get-filter', function (event, data) {
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.BaoDuongListDisplay.length; i++) {
                var select = vm.data.BaoDuongListDisplay[i];
                if (select.isSelected) {
                    checkKhoaSoLieuNam(select.NgayBaoDuong.substring(0, 4)).then(function (success) {
                        BaoDuongListSelected.push(select.BaoDuongId);
                        ...
                        utility.AlertError('Số liêu năm ' + select.NgayBaoDuong.substring(0, 4) + ' đã bị khóa. Vui lòng kiểm tra lại !');
                        return;
                    });
                }
            }
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'BaoDuongId';
            ...
            BaoDuongService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId, TuNgay, DenNgay).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.BaoDuongListDisplay = success.data.data;
                    vm.data.SuaChuaListDisplay = [];
        ...
        function getPageDetail(BaoDuongId) {
            $('tr').removeClass('info');
            $('#row_' + BaoDuongId).addClass('info');
            if (BaoDuongId && BaoDuongId > 0) {
                BaoDuongService.GetPageDetail(BaoDuongId).then(function (success) {
                    if (success.data.data) {
                        $('#bgloadding').remove();
                        vm.data.SuaChuaListDisplay = success.data.data;
