(function () {
    ...
    angular.module("app")
        .controller("DieuChuyenListCtrl", controller)
    ...
    function controller($rootScope, $scope, DieuChuyenService, TuyChonCotService) {
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                if (vm.data.listQuyenTacVu.indexOf("N") > 0) {
                    window.location.href = vm.data.linkUrl + 'DieuChuyen/edit?id=0';
                }
            },
        ...
        vm.data = {
            ...
            listQuyenTacVu: [],
            DieuChuyenListDisplay: [],
            DieuChuyenChiTietListDisplay: [],
            DieuChuyenSelected: [],
            isLoading: false,
            TongSo: 0,
            searchString: '',
        ...
        function loadCotList() {
            if (1 === 1) {
                TuyChonCotService.getAll('FL0021', vm.data.userInfo.UserId).then(function (success) {
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.DieuChuyen.DieuChuyen.reload', function (event) {
                getPage(_tableState);
            });

            $scope.$on('DieuChuyenListCtrl.action.get-filter', function (event, data) {
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'DieuChuyenId';
            ...
            DieuChuyenService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId, TuNgay, DenNgay, SoPhieu).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.DieuChuyenListDisplay = success.data.data;
                    vm.data.DieuChuyenChiTietListDisplay = [];
        ...
        function getPageDetail(DieuChuyenId) {
            $('tr').removeClass('info');
            $('#row_' + DieuChuyenId).addClass('info');
            if (DieuChuyenId && DieuChuyenId > 0) {
                DieuChuyenService.GetPageDetail(DieuChuyenId).then(function (success) {
                    if (success.data.data) {
                        $('#bgloadding').remove();
                        vm.data.DieuChuyenChiTietListDisplay = success.data.data;
