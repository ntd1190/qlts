...
app.controller('LoaiTaiSanCombobox', function ($rootScope, $scope, LoaiTaiSanService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'LoaiTaiSanCombobox';
    vm.NameId = 'cbxLoaiTaiSan';
    vm.data = {
        LoaiTaiSanList: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxLoaiTaiSan.cbxLoaiTaiSan.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.LoaiTaiSan);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        LoaiTaiSanService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
