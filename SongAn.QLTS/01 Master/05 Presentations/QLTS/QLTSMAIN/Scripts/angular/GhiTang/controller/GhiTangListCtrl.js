(function () {
    ...
    angular.module("app")
        .controller("GhiTangListCtrl", controller)
    ...
    function controller($rootScope, $scope, GhiTangService, TuyChonCotService, $q, KhoaSoLieuService, utility) {
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                if (vm.data.listQuyenTacVu.indexOf("N") > 0) {
                    window.location.href = vm.data.linkUrl + 'ghitang/edit?id=0';
                }
            },
        ...
        function loadCotList() {
            if (1 === 1) {
                TuyChonCotService.getAll('FL0019', vm.data.userInfo.UserId).then(function (success) {
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.GhiTang.GhiTang.reload', function (event) {
                getPage(_tableState);
            });
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.GhiTangListDisplay.length; i++) {
                var select = vm.data.GhiTangListDisplay[i];
                if (select.isSelected) {
                    checkKhoaSoLieuNam(select.NgayGhiTang.substring(0, 4)).then(function (success) {
                        GhiTangListSelected.push(select.GhiTangId);
                        ...
                        utility.AlertError('Số liêu năm ' + select.NgayGhiTang.substring(0, 4) + ' đã bị khóa. Vui lòng kiểm tra lại !');
                        return;
                    });
                }
            }
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'GhiTangId';
            ...
            GhiTangService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId, TuNgay, DenNgay, SoPhieu).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.GhiTangListDisplay = success.data.data;
                    vm.data.GhiTangChiTietListDisplay = [];
        ...
        function getPageDetail(GhiTangId) {
            $('tr').removeClass('info');
            $('#row_' + GhiTangId).addClass('info');
            if (GhiTangId && GhiTangId > 0) {
                GhiTangService.GetPageDetail(GhiTangId).then(function (success) {
                    if (success.data.data) {
                        $('#bgloadding').remove();
                        vm.data.GhiTangChiTietListDisplay = success.data.data;
