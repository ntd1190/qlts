(function () {
    ...
    angular.module("app")
        .controller("KhoPhieuNhapListCtrl", controller)
    ...
    function controller($rootScope, $scope, KhoPhieuNhapService, TuyChonCotService) {
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                if (vm.data.listQuyenTacVu.indexOf("N") > 0) {
                    window.location.href = vm.data.linkUrl + 'KhoPhieuNhap/edit?id=0';
                }
            },
        ...
        vm.data = {
            ...
            listQuyenTacVu: [],
            KhoPhieuNhapListDisplay: [],
            KhoPhieuNhapChiTietListDisplay: [],
            KhoPhieuNhapSelected: [],
            isLoading: false,
            TongSo: 0,
            searchString: '',
            KhoTaiSanIds: '',
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.KhoPhieuNhap.KhoPhieuNhap.reload', function (event) {
                getPage(_tableState);
            });

            $scope.$on('KhoPhieuNhapListCtrl.action.get-filter', function (event, data) {
                vm.data.KhoTaiSanIds = data.khoTaiSanId;
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.KhoPhieuNhapListDisplay.length; i++) {
                var select = vm.data.KhoPhieuNhapListDisplay[i];
                if (select.isSelected) {
                    KhoPhieuNhapListSelected.push(select.KhoPhieuNhapId);
                }
            }
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'KhoPhieuNhapId';
            ...
            KhoPhieuNhapService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId, TuNgay, DenNgay, KhoTaiSanId, SoPhieu).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.KhoPhieuNhapListDisplay = success.data.data;
                    vm.data.KhoPhieuNhapChiTietListDisplay = [];
        ...
        function getPageDetail(KhoPhieuNhapId) {
            $('tr').removeClass('info');
            $('#row_' + KhoPhieuNhapId).addClass('info');
            if (KhoPhieuNhapId && KhoPhieuNhapId > 0) {
                KhoPhieuNhapService.GetPageDetail(KhoPhieuNhapId).then(function (success) {
                    if (success.data.data) {
                        $('#bgloadding').remove();
                        vm.data.KhoPhieuNhapChiTietListDisplay = success.data.data;
