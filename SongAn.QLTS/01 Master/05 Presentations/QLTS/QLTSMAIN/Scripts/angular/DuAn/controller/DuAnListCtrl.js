(function () {
    ...
    angular.module("app")
        .controller("DuAnListCtrl", controller)

    function controller($rootScope, $scope, DuAnService) {
        var vm = this;
        //HOT-KEY       
        vm.keys = {
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.DuAn.DuAn.reload', function (event) {
                getPage(_tableState);
            });
        }
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.DuAnListDisplay.length; i++) {
                var DuAn = vm.data.DuAnListDisplay[i];
                if (DuAn.isSelected) {
                    DuAnSelected.push(DuAn.DuAnId);
                }
            }
            var ids = DuAnSelected.join(',');

            DuAnService.removeList(ids).then(function (success) {
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'DuAnId';
            ...
            DuAnService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId).then(function (success) {
        ...
        function edit(id) {
            $rootScope.$broadcast('DuAnEditCtrl.DuAnId', id);
            $('#DuAnEditPopup').collapse('show');
            $("#txtMa").focus();
            $rootScope.isOpenPopup = true;
        }
        ...
        function apDung() {
            ...
            var DuAnListDisplay = vm.data.DuAnListDisplay;
            for (var i = 0; i < DuAnListDisplay.length; i++) {
                if (DuAnListDisplay[i].isSelected) {
                    selectedList.push(DuAnListDisplay[i]);
                }
            }
            console.log(selectedList);
            $scope.$emit('DuAnListPopup.action.ap-dung', selectedList);
        }
