...
app.controller('DuAnCombobox', function ($rootScope, $scope, DuAnService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'DuAnCombobox';
    vm.NameId = 'cbxDuAn';
    vm.data = {
        DuAnList: [],
        DuAnListDisplay: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxDuAn.cbxDuAn.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.DuAn);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        DuAnService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
