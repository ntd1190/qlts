...
app.controller('NhaCungCapCombobox', function ($rootScope, $scope, NhaCungCapService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'NhaCungCapCombobox';
    vm.NameId = 'cbxNhaCungCap';
    vm.data = {
        NhaCungCapList: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxNhaCungCap.cbxNhaCungCap.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.NhaCungCap);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        NhaCungCapService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
