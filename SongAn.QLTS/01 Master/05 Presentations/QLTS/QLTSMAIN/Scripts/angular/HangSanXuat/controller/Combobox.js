...
app.controller('HangSanXuatCombobox', function ($rootScope, $scope, HangSanXuatService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'HangSanXuatCombobox';
    vm.NameId = 'cbxHangSanXuat';
    vm.data = {
        HangSanXuatList: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxHangSanXuat.cbxHangSanXuat.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.HangSanXuat);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        HangSanXuatService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
