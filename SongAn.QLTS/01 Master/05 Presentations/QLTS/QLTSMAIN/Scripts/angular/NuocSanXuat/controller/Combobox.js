...
app.controller('NuocSanXuatCombobox', function ($rootScope, $scope, NuocSanXuatService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'NuocSanXuatCombobox';
    vm.NameId = 'cbxNuocSanXuat';
    vm.data = {
        NuocSanXuatList: [],
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxNuocSanXuat.cbxNuocSanXuat.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.NuocSanXuat);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        NuocSanXuatService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
