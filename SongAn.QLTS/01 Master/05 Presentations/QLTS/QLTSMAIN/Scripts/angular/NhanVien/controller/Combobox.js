...
app.controller('NhanVienCombobox', function ($rootScope, $scope, NhanVienService) {
    var vm = this;
    var userInfo;
    vm.controllerId = 'NhanVienCombobox';
    vm.NameId = 'cbxNhanVien';
    vm.data = {
        NhanVienList: [],
        NhanVienListDisplay: [],
        NhanVien: {},
    ...
    function eventAutoReload() {
        $scope.$on('sa.qltsmain.cbxNhanVien.cbxNhanVien.reload', function (event) {
            getPage();
        });
    }
    function OnClickSelect() {
        $scope.$emit(vm.controllerId + '.action.SelectData', vm.data.NhanVien);
    }
    function getPage() {
        var CoSoId = userInfo.CoSoId;
        var NhanVienId = userInfo.NhanVienId;
        var Search = '';
        NhanVienService.getCombobox(CoSoId, NhanVienId,Search).then(function (success) {
