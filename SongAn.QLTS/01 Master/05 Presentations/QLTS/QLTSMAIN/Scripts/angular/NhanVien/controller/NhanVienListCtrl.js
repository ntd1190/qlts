(function () {
    ...
    function controller($rootScope, $scope, NhanVienService, utility) {
        ...
        vm.keys = {
            ...
            F2: function (name, code) {                
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    $rootScope.$broadcast('NhanVienEditCtrl.NhanVienId', 0);
                    $rootScope.$broadcast('NhanVienEditCtrl.onInitView', 0);
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.NhanVien.NhanVien.reload', function (event) {
                getPage(_tableState);
            });
        }
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.NhanVienListDisplay.length; i++) {
                var NhanVien = vm.data.NhanVienListDisplay[i];
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'NhanVienId';
            ...
            NhanVienService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId).then(function (success) {
        ...
        function edit(id) {
            $rootScope.$broadcast('NhanVienEditCtrl.NhanVienId', id);
            $('#NhanVienEditPopup').collapse('show');
            $("#txtMa").focus();
            $rootScope.isOpenPopup = true;
        }
