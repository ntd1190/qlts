(function () {
    ...
    function controller($rootScope, $scope, BienBanKiemKeService, TuyChonCotService, utility) {
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                if (vm.data.listQuyenTacVu.indexOf("N") > 0) {
                    window.location.href = vm.data.linkUrl + 'BienBanKiemKe/edit?id=0';
                }
            },
        ...
        vm.data = {
            ...
            listQuyenTacVu: [],
            BienBanKiemKeListDisplay: [],
            BienBanKiemKeChiTietListDisplay: [],
            BienBanKiemKeSelected: [],
            isLoading: false,
            TongSo: 0,
            searchString: '',
            TuNgay: '',
        ...
        function loadCotList() {
            if (1 === 1) {
                TuyChonCotService.getAll('FL0024', vm.data.userInfo.UserId).then(function (success) {
        ...
        function eventAutoReload() {
            $scope.$on('sa.qltsmain.BienBanKiemKe.BienBanKiemKe.reload', function (event) {
                getPage(_tableState);
            });

            $scope.$on('BienBanKiemKeListCtrl.action.get-filter', function (event, data) {
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.BienBanKiemKeListDisplay.length; i++) {
                var select = vm.data.BienBanKiemKeListDisplay[i];
                if (select.isSelected) {
                    BienBanKiemKeListSelected.push(select.BienBanKiemKeId);
                }
            }
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'BienBanKiemKeId';
            ...
            BienBanKiemKeService.getPage(draw, start, number, searchString, sortName, sortDir, CoSoId, NhanVienId, TuNgay, DenNgay, SoPhieu, PhongBanId).then(function (success) {
                if (success.data.data) {
                    $('#bgloadding').remove();
                    vm.data.BienBanKiemKeListDisplay = success.data.data;
                    vm.data.BienBanKiemKeChiTietListDisplay = [];
