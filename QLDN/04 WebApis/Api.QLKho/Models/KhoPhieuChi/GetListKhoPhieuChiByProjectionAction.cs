...
using SongAn.QLDN.Biz.QLKho.KhoPhieuChi;
using System.Linq;
using System.Globalization;

namespace SongAn.QLDN.Api.QLKho.Models.KhoPhieuChi
{
    public class GetListKhoPhieuChiByProjectionAction
    {
        ...
        {
            GetListKhoPhieuChiByCriteriaBiz biz = new GetListKhoPhieuChiByCriteriaBiz(context);
            ...
            try
            {
                 ...
                 * fixed input
                 * ========================= */
                sortName = string.IsNullOrEmpty(sortName) ? "PhieuChiId" : sortName;
                ...
                biz.Skip = _start;
                biz.Take = _length;
                IEnumerable<dynamic> listPhieuChi = await biz.Execute();
                if (listPhieuChi.Count() > 0)
                {
                    var obj = listPhieuChi.FirstOrDefault();
                ...
                return ActionHelper.returnActionResult(HttpStatusCode.OK, listPhieuChi, _metaData);
            }
            catch (Exception ex)
            {
