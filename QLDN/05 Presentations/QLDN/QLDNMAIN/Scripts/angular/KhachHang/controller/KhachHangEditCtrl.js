(function () {
    ...
    function KhachHangEditCtrl($rootScope, $scope, KhachHangService, utility, $window, $filter) {
        ...
        function insert() {
            ...
            vm.status.isInValidTinh = utility.checkInValid(vm.data.objKhachHang.Tinh, 'isEmpty');
            if (vm.status.isInValidTinh) {
                return;
            }
            vm.status.isInValidHuyen = utility.checkInValid(vm.data.objKhachHang.Huyen, 'isEmpty');
            if (vm.status.isInValidHuyen) {
                return;
            }
            vm.status.isInValidPhuongXa = utility.checkInValid(vm.data.objKhachHang.Xa, 'isEmpty');
            if (vm.status.isInValidPhuongXa) {
                return;
            }
        ...
        function update() {
            ...
            vm.status.isInValidTinh = utility.checkInValid(vm.data.objKhachHang.TinhThanhPhoId, 'isEmpty');
            if (vm.status.isInValidTinh) {
                return;
            }
            vm.status.isInValidHuyen = utility.checkInValid(vm.data.objKhachHang.QuanHuyenId, 'isEmpty');
            if (vm.status.isInValidHuyen) {
                return;
            }
            vm.status.isInValidPhuongXa = utility.checkInValid(vm.data.objKhachHang.PhuongXaId, 'isEmpty');
            if (vm.status.isInValidPhuongXa) {
                return;
            }
