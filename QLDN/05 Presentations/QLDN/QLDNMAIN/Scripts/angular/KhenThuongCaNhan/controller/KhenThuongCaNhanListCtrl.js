(function () {
    ...
    angular
        .module('app')
        .controller('KhenThuongCaNhanListCtrl', controller)
    ...
    //end HOT-KEY
    function controller($rootScope, $scope, KhenThuongCaNhanService, $window) {
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                ...
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    $rootScope.$broadcast('KhenThuongCaNhanEditCtrl.KhenThuongCaNhanId', 0);
                    $rootScope.$broadcast('KhenThuongCaNhanEditCtrl.onInitView', 0);
                    $('#popupThongTinKhenThuongCaNhan').collapse('show');
                    $("#btnXoaId").hide();
                    $("#btnH").hide();
        ...
        vm.data = {
            ...
            listQuyenTacVu: [],
            KhenThuongCaNhanList: [],
            KhenThuongCaNhanListDisplay: [],
            KhenThuongCaNhanSelected: [],
            NhanVienList: [],
            isLoading: false,
            searchString: '',
            hinhThuc: {
        ...
        function eventAutoReload() {
            ...
            $scope.$on('KhenThuongCaNhanListCtrl.data.ClearHinhThuc', function (event, data) {
                checkHinhThucTatCa(data);
            });
        ...
        function deleteSelected(id) {
            ...
            if (id != '' && id != null) {
                ...
                for (var i = 0; i < vm.data.KhenThuongCaNhanListDisplay.length; i++) {
                    var khenthuong = vm.data.KhenThuongCaNhanListDisplay[i];
                    if (khenthuong.isSelected) {
                        khenThuongSelected.push(khenthuong.KhenThuongCaNhanId);
                    }
                }
            ...
            if (ids != "") {
                ...
                KhenThuongCaNhanService.removeList(ids).then(function (success) {
        ...
        function search() {
            ...
            vm.data.searchString = NhanVienId + '|' + $('#tungay').val() + '|' + $('#denngay').val() + '|' + hinhthuc;
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'KhenThuongCaNhanId';
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = vm.data.searchString;
            var fields = "KhenThuongCaNhanId,Ngay,ho +' ' +ten as HoTen,Tien,HinhThuc,LyDo";
            KhenThuongCaNhanService.getPage(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
                if (success.data.data) {
                    vm.data.KhenThuongCaNhanListDisplay = success.data.data;
        ...
        function add() {
            $rootScope.isSelectAll = true;// using for modal search NhanVien (true: show the checkbox column, false: hide the checkbox column)
            $rootScope.isOpenPopup = true;
            $rootScope.$broadcast('NhanVienListCtrl.action.get-filters', '');
            $rootScope.$broadcast('KhenThuongCaNhanEditCtrl.KhenThuongCaNhanId', 0);
            $rootScope.$broadcast('KhenThuongCaNhanEditCtrl.onInitView', 0);
            $('#popupThongTinKhenThuongCaNhan').collapse('show');
        ...
        function edit(id) {
            $rootScope.isSelectAll = false; // using for modal search NhanVien (true: show the checkbox column, false: hide the checkbox column)
            $rootScope.$broadcast('NhanVienListCtrl.action.get-filters', '');
            $rootScope.$broadcast('KhenThuongCaNhanEditCtrl.KhenThuongCaNhanId', id);
            $('#popupThongTinKhenThuongCaNhan').collapse('show');
            $('#popupNhanVienListmoi').collapse('hide');
            $("#btnXoaId").show();
            $("#btnH").show();
        ...
        function GetListLuocSu(idKhenThuong, tableState) {
            ...
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = idKhenThuong + "|KhenThuongCaNhan";
            var fields = "ngay,sukien, HoTen";
            KhenThuongCaNhanService.getListLuocSu(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
