(function () {
    ...
    angular
        .module('app')
        .controller('KyLuatListCtrl', controller)
    ...
    //end HOT-KEY
    function controller($rootScope, $scope, KyLuatService, $window) {
        var controllerId = 'KyLuatListCtrl';
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                ...
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    $rootScope.$broadcast('KyLuatEditCtrl.KyLuatId', 0);
                    $rootScope.$broadcast('KyLuatEditCtrl.onInitView', 0);
                    $('#popupThongTinKyLuat').collapse('show');
                    $("#btnXoaId").hide();
                    $("#btnH").hide();
        ...
        function eventAutoReload() {
            $scope.$on('sa.qldnmain.kyluat.kyluat.reload', function (event) {
                getPage(_tableState);
            });
        ...
        function checkHinhThucTatCa(status) {
            ...
            if (vm.data.hinhThuc.tatCa == true) {
                vm.data.hinhThuc.phatTien = false;
                vm.data.hinhThuc.canhCao = false;
                vm.data.hinhThuc.vanBan = false;
            }
            else {
                vm.data.hinhThuc.phatTien = true;
            }
        }
        function checkHinhThuc() {

            if (vm.data.hinhThuc.phatTien != true
                && vm.data.hinhThuc.canhCao != true
                && vm.data.hinhThuc.vanBan != true) {
        ...
        function deleteSelected(id) {
            ...
            if (id != '' && id != null) {
                ...
                for (var i = 0; i < vm.data.KyLuatListDisplay.length; i++) {
                    var kyluat = vm.data.KyLuatListDisplay[i];
                    if (kyluat.isSelected) {
                        kyluatSelected.push(kyluat.KyLuatId);
                    }
                }
                ids = kyluatSelected.join(',');
                msg = 'Bạn có muốn xóa các mục đã chọn không?';
            }
            if (ids != "") {
                if (!confirm(msg)) { return; }
                KyLuatService.removeList(ids).then(function (success) {
        ...
        function search() {
            ...
            var hinhthuc = "";
            if (vm.data.hinhThuc.phatTien) hinhthuc = hinhthuc + "1,";
            if (vm.data.hinhThuc.canhCao) hinhthuc = hinhthuc + "2,";
            if (vm.data.hinhThuc.vanBan) hinhthuc = hinhthuc + "3,";
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'KyLuatId';
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = vm.data.searchString;
            var fields = "KyLuatId,Ngay,ho +' ' +ten as HoTen,Tien,HinhThuc,LyDo";
            KyLuatService.getPage(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
                if (success.data.data) {
                    vm.data.KyLuatListDisplay = success.data.data;
        ...
        function edit(data) {
            ...
            $rootScope.$broadcast('KyLuatEditCtrl.KyLuatId', data);
            $('#popupThongTinKyLuat').collapse('show');
            $("#btnXoaId").show();
            $("#btnH").show();
        ...
        function GetListLuocSu(idKyLuat, tableState) {
            vm.data.isLoading = true;
            if (tableState) {
                _tableState = tableState;
            }
            ...
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = idKyLuat + "|KyLuat";
            var fields = "ngay,sukien, HoTen";
            KyLuatService.getListLuocSu(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
