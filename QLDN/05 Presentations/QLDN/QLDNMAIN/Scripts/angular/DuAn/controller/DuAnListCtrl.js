(function () {
    ...
    angular
        .module('app')
        .controller('DuAnListCtrl', DuAnListCtrl)
    ...
    //end HOT-KEY
    function DuAnListCtrl($rootScope, $scope, DuAnService, TuyChonCotService) {
        /* =======================================
         * PRIVATE
         */
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                ...
                if (!$rootScope.isOpenPopup && vm.data.showButtonNew) {
                    $rootScope.$broadcast('DuAnEditCtrl.DuAnId', 0);
                    $rootScope.$broadcast('DuAnEditCtrl.onInitView', 0);
        ...
        function xemDuAn(id) {
            $rootScope.$broadcast(controllerId + '.action.xemDuAn', id);
            $("#btnXoaId").show();
            $("#btnH").show();
        }
        ...
        function loadCotList() {
            if (vm.data.useCotListDb) {
                TuyChonCotService.getAll('FL0007').then(function (success) {
        ...
        function deleteSelected(data) {
            ...
            var msg = "";
            var duanSelected = new Array();
            var ids = "";
            if (data) {
                duanSelected.push(data);
                msg = 'Bạn có muốn xóa không?';
            }
            else {
                for (var i = 0; i < vm.data.listDuAn.length; i++) {
                    var duan = vm.data.listDuAn[i];
                    if (duan.isSelected) {
                        duanSelected.push(duan);
                    }
                }
                ids = duanSelected.join(',');
                msg = 'Bạn có muốn xóa các mục đã chọn không?';
            }
            if (duanSelected.length > 0) {
                for (var i = 0; i < duanSelected.length; i++) {
                    var duan = duanSelected[i];
                    if (duan.NguoiTao != vm.data.UserLoginId) {
                        alert('Bạn không có quyền xóa!')
                        return;
                    }
                }
                if (!confirm(msg)) { return; }
                DuAnService.removeList(duanSelected).then(function (success) {
        ...
        function getPage(tableState) {
            ...
            var fields = "";
            DuAnService.getPage(draw, start, number, search, sortName, sortDir, fields, vm.data.UserLoginId, searchString).then(function (success) {
        ...
        function GetListLuocSu(idDuAn, tableState) {
            vm.data.isLoading = true;
            if (tableState) {
                _tableState = tableState;
            }
            ...
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = idDuAn + "|DuAn";
            var fields = "ngay,sukien, HoTen";
            DuAnService.getListLuocSu(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
