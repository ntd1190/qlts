(function () {
    ...
    angular
        .module('app')
        .controller('BangLuongNhanVienListCtrl', BangLuongNhanVienListCtrl);

    function BangLuongNhanVienListCtrl($rootScope, $scope, BangLuongNhanVienService, TuyChonCotService) {
        /* =======================================
         * PRIVATE
         */
        var controllerId = 'BangLuongNhanVienListCtrl';
        var _tableState;
        var loginId = 0;
        var error = {
            code: 0
        ...
        vm.action = {
            getPage: getPage,
            search: search,
            add: addBangLuongNhanVien,
            xemBangLuongNhanVien: xemBangLuongNhanVien,
            apDung: apDung,
        ...
        function addBangLuongNhanVien() {
            $scope.$emit(controllerId + '.action.xemBangLuongNhanVien', 0);
        }

        function xemBangLuongNhanVien(id) {
            $scope.$emit(controllerId + '.action.xemBangLuongNhanVien', id);
        }

        function xoaBangLuongNhanVien() {

        }

        function loadCotList() {
            if (vm.data.useCotListDb) {
                TuyChonCotService.getAll('FM0011').then(function (success) {
        ...
        function deleteSelected() {
            ...
            for (var i = 0; i < vm.data.listBangLuongNhanVien.length; i++) {
                var nhanvien = vm.data.listBangLuongNhanVien[i];
            ...
            BangLuongNhanVienService.removeList(nhanVienSelected).then(function (success) {
                console.log(success);
        ...
        function getPage(tableState) {
            ...
            var nhanVienIds = joinStr(inputSearch.listBangLuongNhanVien, 'NhanVienId');
            ...
            // gọi api
            BangLuongNhanVienService.getFilter(data).then(function (success) {
                console.log(success);
                if (success.data.metaData.draw == draw && success.data.data) {
                    vm.data.listBangLuongNhanVien = [];
                    while (success.data.data.length) {
                        vm.data.listBangLuongNhanVien.push(success.data.data.shift());
                    }
        ...
        function getFilter(data) {
            inputSearch.listBangLuongNhanVien = (data && data.listBangLuongNhanVien) ? data.listBangLuongNhanVien : [];
        ...
        function apDung() {
            var selectedListBangLuongNhanVien = new Array();
            for (var i = 0; i < vm.data.listBangLuongNhanVien.length; i++) {
                if (vm.data.listBangLuongNhanVien[i].isSelected) {
                    selectedListBangLuongNhanVien.push(vm.data.listBangLuongNhanVien[i]);
                }
            }
            emitApDung(selectedListBangLuongNhanVien);
        }
        ...
        function initEventListener() {
            ...
            $scope.$on(controllerId + '.action.getInfo', function (event, data) {
                var list = getBangLuongNhanVienInfo(data);
        ...
        function getBangLuongNhanVienInfo(ids) { // ids = '1|2|3'
            var list = [];
            ...
            BangLuongNhanVienService.getFilter(data
