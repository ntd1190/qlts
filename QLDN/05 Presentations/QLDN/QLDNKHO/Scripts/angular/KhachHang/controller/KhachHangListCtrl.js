(function () {
    ...
    //end HOT-KEY
    function KhachHangListCtrl($rootScope, $scope, KhachHangService, TuyChonCotService, utility) {
        /* =======================================
         * PRIVATE
         */
        var vm = this;
        ...
        // 2017.07.25 binhnt update xóa
        function deleteSelected(id) {
            vm.data.isLoading = true;
            var msg = "";
            var khachhangSelected = new Array();
            if (id != '' && id != null) {
                khachhangSelected.push({ KhachHangId: id });
                ...
                for (var i = 0; i < vm.data.listKhachHang.length; i++) {
                    ...
                    if (khachhang.isSelected) {
                        khachhangSelected.push(khachhang);
                    }
                }
                msg = 'Bạn có muốn xóa các mục đã chọn không?';
            }
            if (khachhangSelected.length > 0) {
                if (!confirm(msg)) { return; }

                var data = {};
                data.list = utility.clone(khachhangSelected);
                data.loginId = 0;

                KhachHangService.removeList(data).then(function (success) {
                    console.log(success);
                    ...
                    if (error.status == 400) {
                        ...
                        alert('Khách hàng không thể xóa!')
                    }
                });
            } else {
