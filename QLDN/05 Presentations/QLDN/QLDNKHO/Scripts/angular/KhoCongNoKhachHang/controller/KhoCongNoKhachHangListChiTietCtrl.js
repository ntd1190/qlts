(function () {
    ...
    angular
        .module('app')
        .controller('KhoCongNoKhachHangListChiTietCtrl', KhoCongNoKhachHangListChiTietCtrl)
    ...
    //end HOT-KEY
    function KhoCongNoKhachHangListChiTietCtrl($rootScope, $scope, KhoCongNoKhachHangService, $timeout, ExcelExport) {
        /* =======================================
         * PRIVATE
         */
        var vm = this;
        ...
        var controllerId = 'KhoCongNoKhachHangListChiTietCtrl';
        var _tableState;
        ...
        function onInitView(config) {
            ...
            if (config && config.KhachHangId) {
                vm.data.searchString = $('#tungay').val() + '|' + $('#denngay').val() +'|' + config.KhachHangId;
            }
        ...
        function xemKhoPhieuThu(id) {
            $rootScope.$broadcast('KhoPhieuThuListCtrl.action.xemKhoPhieuThu', id);
        }
        ...
        function getPage(tableState) {
            ...
            var fields = "";
            KhoCongNoKhachHangService.getPageChiTiet(draw, start, number, searchString, sortName, sortDir, fields, vm.data.UserLoginId).then(function (success) {
                if (success.data.data) {
                    vm.data.listKhoCongNoKhachHang = success.data.data;
                    tableState.pagination.numberOfPages = Math.ceil(success.data.metaData.total / number);
                    vm.data.NhaCungCap = vm.data.listKhoCongNoKhachHang[0].Ten;
                    vm.data.TongNhap = 0;
                    vm.data.TongChi = 0;
                    vm.data.NgayKiemKho = searchString.split('|')[0] + ' ~ ' + searchString.split('|')[1];
                    for (var i = 0; i < vm.data.listKhoCongNoKhachHang.length; i++) {
                        var kho = vm.data.listKhoCongNoKhachHang[i];
                        if (kho.NgayXuat != '')
                        {
                            vm.data.TongNhap = vm.data.TongNhap + kho.TienNhap
                            vm.data.TongChi = vm.data.TongChi + kho.TienChi
                        }
                    }
                    vm.data.DuCuoi = vm.data.listKhoCongNoKhachHang[0].TienNhap + vm.data.TongNhap - vm.data.TongChi;
                }
        ...
        function initEventListener() {
            ...
            $scope.$on(controllerId + '.action.get-filters', function (event, data) {
                ...
                vm.data.NgayKiemKho = data.split('|')[0]+' ~ '+data.split('|')[1];
                _tableState.pagination.start = 0;
                getPage(_tableState);
            });
       
            $scope.$on(controllerId + '.action.getInfo', function (event, data) {
                var list = getKhoCongNoKhachHangInfo(data);
