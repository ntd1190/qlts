(function () {
    ...
    angular
        .module('app')
        .controller('KhoPhieuThuListCtrl', KhoPhieuThuListCtrl)
    ...
    //end HOT-KEY
    function KhoPhieuThuListCtrl($rootScope, $scope, KhoPhieuThuService, TuyChonCotService) {
        /* =======================================
         * PRIVATE
         */
        var vm = this;
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                ...
                if (!$rootScope.isOpenPopup) {
                    $rootScope.$broadcast('KhoPhieuThuEditCtrl.KhoPhieuThuId', 0);
                    $rootScope.$broadcast('KhoPhieuThuEditCtrl.onInitView', 0);
        ...
        //end HOT-KEY
        var controllerId = 'KhoPhieuThuListCtrl';
        var _tableState;
        ...
        vm.data = {
            ...
            listQuyenTacVu: [],
            listCot: [
                { MaCot: 'KhoPhieuThuId', TenCot: 'ID', HienThiYN: false, DoRong: 100 },
                { MaCot: 'NgayTao', TenCot: 'Ngày tiếp nhận', HienThiYN: true, DoRong: 100 },
                { MaCot: 'TieuDe', TenCot: 'Tiêu đề', HienThiYN: true, DoRong: 10 },
                { MaCot: 'LoaiKhoPhieuThu', TenCot: 'Loại KhoPhieuThu', HienThiYN: true, DoRong: 10 },
            ],
        ...
        vm.action = {
            ...
            search: search,
            apDung: apDung,
            addKhoPhieuThu: addKhoPhieuThu,
            xemKhoPhieuThu: xemKhoPhieuThu,
        ...
        function checkCot(cot) {
            ...
            switch (cot.MaCot) {
                ...
                case 'KhoPhieuThuId': return false;
                case 'MaTrangThai': return false;
                case 'Ho': return false;
                default: return true;
            }
        }
        function addKhoPhieuThu() {
            $('#popupThongTinKhoPhieuThu').collapse('show');
            $("#btnXoaId").hide();
            $("#btnH").hide();
            $("#txtTieuDe").focus();
        ...
        function xemKhoPhieuThu(id) {
            $rootScope.$broadcast(controllerId + '.action.xemKhoPhieuThu', id);
            $("#btnXoaId").show();
            $("#btnH").show();
        }

        function xoaKhoPhieuThu() {

        }
       
        function loadCotList() {
            if (vm.data.useCotListDb) {
                TuyChonCotService.getAll('FM0013').then(function (success) {
        ...
        function deleteSelected(data) {
            ...
            if (data) {
                ...
                for (var i = 0; i < vm.data.listKhoPhieuThu.length; i++) {
                    var congviec = vm.data.listKhoPhieuThu[i];
            ...
            if (congviecSelected.length > 0) {
                ...
                KhoPhieuThuService.removeList(congviecSelected).then(function (success) {
        ...
        function getPage(tableState) {
            ...
            var sortName = tableState.sort.predicate || 'A.PhieuThuId';
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = vm.data.searchString;
            var fields = "";
            KhoPhieuThuService.getPage(draw, start, number, searchString, sortName, sortDir, fields,vm.data.UserLoginId).then(function (success) {
                if (success.data.data) {
                    vm.data.listKhoPhieuThu = success.data.data;
        ...
        function initEventListener() {
            ...
            $scope.$on(controllerId + '.action.getInfo', function (event, data) {
                var list = getKhoPhieuThuInfo(data);
                $scope.$emit(controllerId + '.data.listInfo', list);
            });
        }

        function apDung() {
            var selectedListKhoPhieuThu = new Array();
            for (var i = 0; i < vm.data.listKhoPhieuThu.length; i++) {
                if (vm.data.listKhoPhieuThu[i].isSelected) {
                    selectedListKhoPhieuThu.push(vm.data.listKhoPhieuThu[i]);
                }
            }
            emitApDung(selectedListKhoPhieuThu);
        }
        ...
        function GetListLuocSu(idKhoPhieuThu, tableState) {
            vm.data.isLoading = true;
            if (tableState) {
                _tableState = tableState;
            }
            ...
            var sortDir = tableState.sort.reverse ? 'desc' : 'asc';
            var searchString = idKhoPhieuThu + "|KhoPhieuThu";
            var fields = "ngay,sukien, HoTen";
            KhoPhieuThuService.getListLuocSu(draw, start, number, searchString, sortName, sortDir, fields).then(function (success) {
