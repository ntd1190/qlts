...
(function () {
    ...
    function MainCtrl($scope, $rootScope, $http) {
        ...
        vm.controllerId = {
            ...
            KhoPhieuNhapList: 'KhoPhieuNhapList',
            KhoPhieuNhapFilter: 'KhoPhieuNhapFilter',
            TuyChonCotPopup: 'TuyChonCotPhieuNhapPopup',
        }
        ...
        vm.keys = {
            ...
            F2: function (name, code) {
                console.log('F2');
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapList + '.action.F2');
            },
            ...
            F8: function (name, code) {
                //alert("F8");
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapFilter + '.action.F8');
        ...
        function TuyChonCotPopupEvent() {
            ...
            $scope.$on(vm.controllerId.TuyChonCotPopup + '.action.ap-dung', function (event, data) {
                ...
                $rootScope.$broadcast(vm.controllerId.KhoPhieuNhapList + '.action.reload');
            });
        ...
        function catchKhachHangListPopupEvent() {
            $scope.$on(vm.controllerId.KhachHangListPopup + '.action.ap-dung', function (e, v) {
                ...
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapFilter + '.data.listKhachHang', data);
                $('#' + vm.controllerId.KhachHangListPopup).collapse('hide');
            });
        }
        function catchKhoPhieuNhapFilterEvent() {
            $scope.$on(vm.controllerId.KhoPhieuNhapFilter + '.data.filter', function (e, v) {
                console.log(v);
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapList + '.data.filter', v);
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapList + '.action.reload');
            });
        }
        ...
        function KhoKhoHangListPopupEvent() {
            $scope.$on(vm.controllerId.KhoKhoHangListPopup + '.action.ap-dung', function (e, v) {
                console.log(v);
                $scope.$broadcast(vm.controllerId.KhoPhieuNhapFilter + '.data.listKhoHang', v);
