...
{
    public class DeleteKeHoachByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteKeHoachByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.KeHoachId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("KeHoachId không hợp lệ");
                }
            }
        }
