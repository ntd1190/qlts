...
{
    public class DeleteChiTieuByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteChiTieuByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.ChiTieuId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("ChiTieuId không hợp lệ");
                }
            }
        }
