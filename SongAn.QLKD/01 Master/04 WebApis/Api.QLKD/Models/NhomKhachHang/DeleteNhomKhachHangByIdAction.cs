...
{
    public class DeleteNhomKhachHangByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteNhomKhachHangByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.NhomKhachHangId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("NhomKhachHangId không hợp lệ");
                }
            }
        }
