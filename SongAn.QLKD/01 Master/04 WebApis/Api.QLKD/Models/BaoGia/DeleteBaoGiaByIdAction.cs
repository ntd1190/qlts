...
{
    public class DeleteBaoGiaByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteBaoGiaByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.BaoGiaId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("BaoGiaId không hợp lệ");
                }
            }
        }
