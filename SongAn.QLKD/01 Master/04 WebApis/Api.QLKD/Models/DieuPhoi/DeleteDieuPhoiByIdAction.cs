...
{
    public class DeleteDieuPhoiByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteDieuPhoiByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.DieuPhoiId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("DieuPhoiId không hợp lệ");
                }
            }
        }
