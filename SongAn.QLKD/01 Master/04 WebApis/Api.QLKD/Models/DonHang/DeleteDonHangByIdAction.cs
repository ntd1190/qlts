...
{
    public class DeleteDonHangByIdAction
    {
        ...
        public async Task<ActionResultDto> Execute(ContextDto context)
        {
            try
            {
                ...
                var biz = new DeleteDonHangByIdBiz(context);
                for (int i = 0; i < _listId.Count; i++)
                {
                    biz.DonHangId = Protector.Int(_listId[i]);
        ...
        private void validate()
        {
            for (int i = 0; i < _listId.Count; i++)
            {
                if (_listId[i] < 1)
                {
                    throw new FormatException("DonHangId không hợp lệ");
                }
            }
        }
