...
5. Example		: 
					...
					EXEC sp_KD_DuLieu_cbxGetListByCriteria
						 @DULIEUID		=	NULL
						,@SEARCH		=	N''
...
***************************************************/
ALTER PROCEDURE sp_KD_DuLieu_cbxGetListByCriteria
	 @DULIEUID		INT					=	NULL
...
IF @ORDERCLAUSE = ''
	SET @ORDERCLAUSE = 'DL.DuLieuId'

-- SELECT
SET @V_SQL = N'
	SELECT COUNT(DL.DuLieuId) OVER () AS MAXCNT,DL.*,DL.DuLieuId ID
	FROM KDDuLieu DL
'

-- WHERE
SET @V_SQL = @V_SQL + N'
	WHERE (@DULIEUID = 0 OR DL.DuLieuId = @DULIEUID)
		AND (@SEARCH = '''' 
			OR DL.MaDuLieu LIKE @V_SEARCH
			OR DL.TenDuLieu LIKE @V_SEARCH
			OR DL.ViTri LIKE @V_SEARCH
		)
'
