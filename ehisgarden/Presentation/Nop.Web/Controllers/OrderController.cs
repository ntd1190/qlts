...
using Nop.Web.Models.BaoHanh.ViewBaoHanh;
...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        //My account / Orders
        [NopHttpsRequirement(SslRequirement.Yes)]
        public virtual ActionResult CustomerOrders1(int id)
        {
            var model = _orderModelFactory.PrepareCustomerOrderListModel1(id);
            return View(model);
        }
        [NopHttpsRequirement(SslRequirement.Yes)]
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult CustomerList(string phone)
        {
            var value = removeXss(phone);
            using (var db = new Nop.Web.Models.BaoHanh.MSSQL_QLDN_QLNS_DEMOEntities())
            {
                try
                {
                    var mo = db.Database.SqlQuery<Baohanhmodel>("exec dbo.[sp_KhoPhieuBaoHanh_GetThongTinBySeries] @LOGIN_ID=Null,@MESSAGE=Null,@SERIES=" + value + "").ToList();
                    if (!mo.Any())
                    {
                      mo= db.Database.SqlQuery<Baohanhmodel>("exec dbo.[sp_KhoPhieuBaoHanh_GetThongTinByDienThoai] @DIENTHOAI=" + phone+ ",@LOGIN_ID=Null,@MESSAGE=Null,@SKIP=0,@TAKE=100").ToList();
                        return View(mo);
                    }
                    else
                    {
                ...
                {
                    return View(new List<Baohanhmodel>());
                }
            }
        }
        ...
        [NopHttpsRequirement(SslRequirement.Yes)]
        public virtual ActionResult Details1(int orderId)
        {
            var order = _orderService.GetOrderById(orderId);
