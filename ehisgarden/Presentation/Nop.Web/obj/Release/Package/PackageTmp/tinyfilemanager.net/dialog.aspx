...
<body>
                    ...
                    <%  
                        ...
                        foreach (TinyFileManager.NET.clsFileItem objF in this.arrLinks)
                        {
                            ...
                            else
                            {
                                ...
                                Response.Write("</div>");
                                Response.Write("</form>");
                                //  Response.Write("<a class=\"abcx\" href=\"#\" title=\"Select\" onclick=\"apply('" + objF.strThumbImage + "',0);\"=\"\"><img  src=\"" + objF.strThumbImage + "\" alt=\"folder\" /><h3>" + objF.strName + "</h3></a>"); anh hòa viết để dự phòng
                                //Response.Write(objF.strLink.Replace(" ","1"));
                                Response.Write(objF.strLink);
                            }
    ...
    <script language="javascript" type="text/javascript">
        ...
        function apply(file, type_file) {
            ...
            <%if (this.objConfig.strFillSelector == "'.image', window.opener.document")
              {%>
                <% Response.Write(this.objConfig.strPopupCloseCode); %>
            var html= '';
            var html= '<div class="thaihoa"><img  id="ImagePath" class="hoadeptrai" name="ImagePath" src='+file+'  value="' + file + '"  style="width: 150px;height: 100px;"/>';
            html+='<input  type="hidden" name="ImagePath" class="PTH" id="multi12" value="' + file + '"/><a  onclick="clickne(this);" class="glyphicon glyphicon-remove hidden-cus"></a></div>';
               
            $(<% Response.Write(this.objConfig.strFillSelector); %>).append(html);
            
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            <%}%>
            <%if (this.objConfig.strFillSelector == "'.image2', window.opener.document")
              {%>
                <% Response.Write(this.objConfig.strPopupCloseCode); %>
            var html= '';
            var html= '<div class="thaihoa"><img  id="ImagePath2" class="hoadeptrai" name="ImagePath2" src='+file+'  value="' + file + '"  style="width: 150px;height: 100px;"/>';
            html+='<input  type="hidden" name="ImagePath2" class="PTH" id="multi12" value="' + file + '"/><a  onclick="clickne(this);" class="glyphicon glyphicon-remove hidden-cus"></a></div>';
            ...
            <%if (this.objConfig.strFillSelector == "'.image3', window.opener.document")
              {%>
                <% Response.Write(this.objConfig.strPopupCloseCode); %>
            var html= '';
            var html= '<div class="thaihoa"><img  id="ImagePath3" class="hoadeptrai" name="ImagePath3" src='+file+'  value="' + file + '"  style="width: 150px;height: 100px;"/>';
            html+='<input  type="hidden" name="ImagePath3" class="PTH" id="multi12" value="' + file + '"/><a  onclick="clickne(this);" class="glyphicon glyphicon-remove hidden-cus"></a></div>';
               
            $(<% Response.Write(this.objConfig.strFillSelector); %>).append(html);
            
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            <%}%>
            <%if (this.objConfig.strFillSelector == "'.file2', window.opener.document")
              {%>
        <% Response.Write(this.objConfig.strPopupCloseCode); %>
            var html= '';
            html+='<input type="text" name="LinkDownload2" value="' + file + '" style="width: 350px;"/>'
               
            $(<% Response.Write(this.objConfig.strFillSelector); %>).append(html);
            
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            <%}%>
            <%if (this.objConfig.strFillSelector == "'.file', window.opener.document")
              {%>
        <% Response.Write(this.objConfig.strPopupCloseCode); %>
            var html= '';
            html+='<input type="text" name="LinkDownload" value="' + file + '" style="width: 350px;"/>'
               
            $(<% Response.Write(this.objConfig.strFillSelector); %>).append(html);
            
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            <%}
              if (this.objConfig.strFillSelector == "'.imagedetail4', window.opener.document")
              {%>
            var html= '<div class="thaihoa"><img  id="ImagePath4" class="hoadeptrai" name="ImagePath4" src='+file+'  value="' + file + '"  style="width: 150px;height: 100px;"/>';
            html+='<input  type="hidden" name="ImagePath4" class="PTH" id="multi12" value="' + file + '"/><a  onclick="clickne(this);" class="glyphicon glyphicon-remove hidden-cus"></a></div>';
               
            $(<% Response.Write(this.objConfig.strFillSelector); %>).append(html);
            
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);<%}

        if (this.objConfig.strFillSelector == "'.imagedetail', window.opener.document")
        {%>
            var html= '<div class="thaihoa"><img  id="ImagePath1" class="hoadeptrai" name="ImagePath1" src='+file+'  value="' + file + '"  style="width: 150px;height: 100px;"/>';
            html+='<input  type="hidden" name="ImagePath1" class="PTH" id="multi12" value="' + file + '"/><a  onclick="clickne(this);" class="glyphicon glyphicon-remove hidden-cus"></a></div>';
