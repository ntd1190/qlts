...
namespace Nop.Web.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public virtual CustomerOrderListModel PrepareCustomerOrderListModel1(int id)
        {
            var model = new CustomerOrderListModel();
            //find customer id
            ...
            if (cus != null)
            {
                ...
                var orders = _orderService.SearchOrders(storeId: _storeContext.CurrentStore.Id, customerId: cus.Id);
                foreach (var order in orders)
                {
                    var orderModel = new CustomerOrderListModel.OrderDetailsModel
                    {
