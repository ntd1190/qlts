    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
                            ...
                            if (!product.CustomerEntersPrice)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            var prdau = _priceFormatter.FormatPrice(product.Price);
                                            var final = _priceFormatter.FormatPrice(finalPrice);
                                            if (final == prdau)
                                            {
                                                priceModel.OldPrice = null;
                                                priceModel.Price = _priceFormatter.FormatPrice(finalPrice);
                                            ...
                                            {
                                                priceModel.PriceDiscount = "-" +  Convert.ToString((Convert.ToDecimal(prdau) - Convert.ToDecimal(final))*100 / Convert.ToDecimal(prdau)) + "%";
                                                priceModel.OldPrice = _priceFormatter.FormatPrice(product.Price);
                                                priceModel.Price = _priceFormatter.FormatPrice(finalPrice);
                                                priceModel.PriceValue = finalPrice;
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                Name = product.GetLocalized(x => x.Name),
                ShortDescription = product.GetLocalized(x => x.ShortDescription),
                DigitalDescription=product.GetLocalized(x=>x.DigitalDescription),
                Guarantee=product.GetLocalized(x=>x.Guarantee),
                FullDescription = product.GetLocalized(x => x.FullDescription),
